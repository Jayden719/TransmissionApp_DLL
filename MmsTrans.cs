...
{
    public partial class MmsTrans : Form
    {
        //SMS 전송 마샬링
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                //SMS 전송
                result = MoashotSendSMS(suserid, spwd, sfrom, sto, scontents, stitle, sindexcode, stime);
            }
            else if(Int32.Parse(label10.Text) > 90 && Int32.Parse(label10.Text) <= 2000)
            {
                ...
                {
                    //LMS 전송
                    result = MoashotSendLMS(suserid, spwd, sfrom, sto, scontents, stitle, sindexcode, stime);
                }
                else
                {
                    //MMS 전송
           ...
           /* if (Regex.IsMatch(scontents, @"[ㄱ-ㅎ가-힣]"))
            ...
            if (result == 0)
            {
                ...
                {
                    File.AppendAllText(logPath, "\r\n" + CurTime + " MMS " + " 발송번호 " + sfrom + " " + " 수신번호 " + sto + " 발송 완료", Encoding.Default);
                }
                else
                {
                    File.AppendAllText(logPath, "\r\n" + CurTime + " MMS " + " 발송번호 " + sfrom + " " + " 수신번호 " + sto + " " + sRdate + " 예약접수 완료", Encoding.Default);
                }
            }
            else if (result == 1)
            {
