...
{
    public partial class LmsTrans : Form
    {
        ...
        private void BchkMethod(string text, char[] tempChr)
        {
            ...
            if (bCnt > 2000)
            {
                MessageBox.Show("2000바이트 초과되었습니다");
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            int result = MoashotSendLMS(suserid, spwd, sfrom, sto, scontents, stitle, sindexcode, stime);
            if (result == 0)
            {
                label9.Text = "성공";
                if (stime == "" || stime.Length == 0)
                {
                    File.AppendAllText(logPath, "\r\n" + CurTime + " LMS " + " 발송번호 " + sfrom + " " + " 수신번호 " + sto + " 발송 완료", Encoding.Default);
                }
                else
                {
                    File.AppendAllText(logPath, "\r\n" + CurTime + " LMS " + " 발송번호 " + sfrom + " " + " 수신번호 " + sto + " " + sRdate + " 예약접수 완료", Encoding.Default);
                }
            }
            else if (result == 1)
            {
